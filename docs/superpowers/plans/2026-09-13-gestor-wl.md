...
- Modify: `src/main.tsx` (importa os dois CSS)
...
- Produces:
  ...
  - `Tag({ bg, fg, children })` e `TagOutline({ children })` — dois componentes, porque o design system tem uma classe `.tag-outline` própria que não recebe cores
...
**Interfaces:**
- Consumes: `Dialog`, `Field`, `Seg` da Task 14; `EMPTY_ITEM`, `WALLETS`, `TYPES`, `CHAINS`, `STATUSES` de `../lib/types.ts`; `money`, `num` de `@shared/money.ts`; `NEG`, `POS` de `../lib/tokens.ts`.
